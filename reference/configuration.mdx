...
| `~/.gpr/agents.json` | Optional agent launch overrides (command, args, env). See [agents.json](/reference/agents-json) |
...
The built-in catalog is enough for a normal install. Use [agents.json](/reference/agents-json) when you need per-agent environment variables, a custom ACP command, or an agent that is not in the catalog.
